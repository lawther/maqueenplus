...
class MaqueenPlusV2:
	...
	def __init__(A):
		...
		B=C
		while B==C:
			E=A._a();D=E[-3:];A._o=int(D[0]);A._p=int(D[2])
			if A._o==2 and A._p==1:B=True
			if B==C:microbit.display.show(microbit.Image.NO);sleep_ms(1000)
		A._n=NeoPixel(microbit.pin15,A._k);A.motor_stop(A.MOTOR_BOTH);A.set_headlight(A.HEADLIGHT_BOTH,A.LED_OFF);A.set_underglow_off();microbit.display.show(microbit.Image.YES);microbit.display.clear()
	...
	def get_range_cm(B):
		...
		if C.read_digital()==0:A.write_digital(0);A.write_digital(1);sleep_ms(20);A.write_digital(0);E=machine.time_pulse_us(C,1,B._h*58)
		else:A.write_digital(1);A.write_digital(0);sleep_ms(20);A.write_digital(0);E=machine.time_pulse_us(C,0,B._h*58)
		D=E/59
		if D<=0:return 0
		if D>=B._h:return B._h
		return round(D)
	...
	def line_track(B):
		...
		if B._p==0:return A>>0&1==1,A>>1&1==1,A>>2&1==1,A>>3&1==1,A>>4&1==1
